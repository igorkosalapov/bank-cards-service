...
В проекте 27 автоматических тестов:

- 18 unit-тестов сервисного слоя с использованием Mockito;
- 7 MockMvc-тестов безопасности, авторизации и валидации API;
- unit-тест шифрования данных;
- интеграционный тест конкурентного перевода на PostgreSQL Testcontainers.

Интеграционный тест одновременно запускает два перевода по 80 с карты,
баланс которой равен 100. Тест подтверждает, что благодаря
пессимистической блокировке успешен только один перевод, повторного
списания не происходит, а общая сумма денег остаётся неизменной.

### Запуск тестов

Для полного запуска тестов требуется работающий Docker:

```bash
mvn --batch-mode clean verify
```

Testcontainers автоматически запускает временный контейнер
`postgres:16-alpine`, а после завершения тестов удаляет его.

Та же команда выполняется в GitHub Actions при создании Pull Request
и после отправки изменений в `main`.
